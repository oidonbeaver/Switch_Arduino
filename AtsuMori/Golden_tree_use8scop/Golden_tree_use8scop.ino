...
int daynum = 1;   // 1日から始める
int monthnum = 1; //3月から始める
...
int changescop(int scopnum,std::vector<std::vector<int>> direc_mat);
...
void loop()
{
  ...
  if (daynum % 4 == 0)
  {
    movestep(2, 255, 128, 90); //右
    SwitchControlLibrary().PressButtonL();
    delay(50);
    SwitchControlLibrary().PressButtonA();
    delay(50);
    SwitchControlLibrary().ReleaseButtonA();//ゴミ箱選択
    delay(2000);
    SwitchControlLibrary().PressButtonA();//セリフすすめる
    delay(50);
  ...
  movestep(5, 0, 128, 180); //左
  hitstone(5);
  delay(1500);
  movestep(2, 128, 0, 90); //上
  movestep(5, 0, 128, 90); //左
  movestep(5, 0, 180, 90); //左下
  movestep(3, 255, 128, 90); //右
  hitstone(5);
  delay(1500);
...
//  SwitchControlLibrary().PressButtonA();
//  delay(1500);
scopnum = changescop(scopnum,direc_mat);


  pickstone(20, 128, 0); // 上
  movestep(1, 128, 0, 0); //上
  movestep(4, 255, 128, 90); //右
  pickstone(20, 128, 255); // 下
  movestep(1, 128, 255, 0); //下
  pickstone(40, 0, 128); // 左
  movestep(4, 128, 255, 90); //下
  movestep(16,0,128, 90); //左
//
//  movestep(3, 0, 128, 90); //左
//  movestep(3, 128, 0, 90); //上
//  movestep(6, 0, 128, 90);
  movestep(10, 128, 0, 90);
  //最初の石に到着
...
//  pickstone(3, 0, 128); // 左その場で
  ...
  { //28日以外
    ...
    if((monthnum==7)&&(daynum==8)){ //誕生日を避ける
      SwitchControlLibrary().MoveHat(0); //上
      delay(50);
      SwitchControlLibrary().MoveHat(8);
      delay(120);
